...
/**
 * Inline Owl SVG for the loading overlay.
 * White icon with cyan glow animation.
 */
function AnimatedOwl() {
  const size = 96;
  return (
 ...
 * - Centered owl with cyan glow
 * - Gentle float animation
 * - Accessible: aria-busy, doesn't trap focus
 */
export function GlobalLoaderOverlay() {
  const storeVisible = useLoadingStore((state) => state.isVisible);

  // DEBUG: Set to true to test the overlay appearance
  const DEBUG_ALWAYS_SHOW = false;
  const isVisible = DEBUG_ALWAYS_SHOW || storeVisible;
